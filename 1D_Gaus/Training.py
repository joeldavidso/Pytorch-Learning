...
from Config import Data, Network, hyper_params, train_loop, val_loop, CR
...
if not CR:
	...
	train_data = Data(torch.from_numpy(train_file["Gaus"]["x"]).unsqueeze(-1),
	...
	# Converted h5 files for 2b->4b reweighting regions
	train_data = Data(torch.from_numpy(train_file["Gaus"]["x"][train_file["Gaus"]["y"]>0]).unsqueeze(-1),
					torch.from_numpy(train_file["Gaus"]["weight"][train_file["Gaus"]["y"]>0]).unsqueeze(1),
					torch.from_numpy(train_file["Gaus"]["label"][train_file["Gaus"]["y"]>0]).unsqueeze(1))
